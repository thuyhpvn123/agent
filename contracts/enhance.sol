...
contract EnhancedAgentManagement is AgentManagement {
    ...
    function getMeosSCByAgentFromFactory(address _agent, uint _branchId) external view returns (MeosContracts memory) {
        MeosContracts memory meosContracts = IMeosFactory(meosFactory).getMeosSCByAgentFromFactory(_agent,_branchId);
    ...
    function getIQRSCByAgentFromFactory(address _agent, uint _branchId) external view returns (IQRContracts memory) {
        IQRContracts memory iqrContracts = IIQRFactory(iqrFactory).getIQRSCByAgentFromFactory(_agent,_branchId);
        return iqrContracts;
    }
    ...
    function BatchSetAllPointsIQR(address _agent)external onlySuperAdmin{
        ...
        uint[] memory _branchIds = agentToBranchIds[_agent];
        require(_branchIds.length>0,"have no branch id found");
        address POINTS_PROXY;
        for(uint i; i< _branchIds.length; i++){
            POINTS_PROXY =  _setPointsIQR(_agent,_branchIds[i]);
        }
    ...
    function _setPointsIQR(address _agent,uint _branchId) internal returns(address POINTS_PROXY) {
        if (agentLoyaltyContracts[_agent] != address(0) ){
            
            address management;
            address order;
            address topup;
            if(agents[_agent].permissions[0]){
                IIQRFactory(iqrFactory).setPointsIQRFactory(_agent,agentLoyaltyContracts[_agent],_branchId);
                IQRContracts memory iqr = IIQRFactory(iqrFactory).getIQRSCByAgentFromFactory(_agent,_branchId);
                management = iqr.Management;
                order = iqr.Order;
            }
            if(agents[_agent].permissions[2]){
                IMeosFactory(meosFactory).setPointsMeosFactory(_agent,agentLoyaltyContracts[_agent],_branchId);
                MeosContracts memory meos = IMeosFactory(meosFactory).getMeosSCByAgentFromFactory(_agent,_branchId);
                topup = meos.NetCafeTopUp;
            }
            POINTS_PROXY = ILoyaltyFactory(loyaltyFactory).setPointsLoyaltyFactory(_agent,management, order,topup, _branchId);
        }
