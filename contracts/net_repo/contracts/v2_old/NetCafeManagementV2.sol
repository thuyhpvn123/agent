...
contract NetCafeManagementV2 is OwnableUpgradeable, UUPSUpgradeable, NetCafeStaffUpgradeable {
    struct StationMeta {
        bytes32 pcId;
        string name;
        bytes32 groupId;
    ...
    function updatePricePolicy(bytes32 id, uint256 pricePerMinute) external onlyFinanceStaff {
    ...
    function getStationIdsPaged(
        ...
        uint256 total = stationIds.length;
    ...
    function getPricePolicyIdsPaged(
        ...
        uint256 total = pricePolicyIds.length;
        if (offset >= total) return new bytes32[](0);
